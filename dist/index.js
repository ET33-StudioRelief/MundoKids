...
(() => {
  ...
  function initAccordionToggle() {
    const plusSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M12 19.5V4.5M4.5 12H19.5" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path></svg>';
    const minusSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M4.5 12L19.5 12" stroke="white" stroke-width="2" stroke-linecap="round"/></svg>';
    document.addEventListener("click", (event2) => {
      const target = event2.target;
      if (!target) return;
      const trigger = target.closest(".fs_accordion_arrow-wrapper");
      if (!trigger) return;
      const header = trigger.closest(".fs_accordion_header");
      let content = null;
      if (header && header.nextElementSibling instanceof HTMLElement && header.nextElementSibling.matches(".fs_accordion_content")) {
        content = header.nextElementSibling;
      }
      if (!content) {
        const container = trigger.closest(".fs_accordion") || trigger.closest(".fs_accordion_item") || document;
        content = container.querySelector(".fs_accordion_content");
      }
      if (!content) return;
      ...
      const svgHolder = trigger.querySelector(".fs_accordion_show-svg");
      if (svgHolder) {
      ...
      if (gsap3) {
        ...
        if (_body2) {
          ...
          _autoRefresh = [_doc4, "visibilitychange", function() {
            var w2 = _win4.innerWidth, h = _win4.innerHeight;
            if (_doc4.hidden) {
              _prevWidth = w2;
              _prevHeight = h;
  ...
  var gsapCore;
  var ScrollTriggerPlugin;
  var w = typeof window !== "undefined" ? window : void 0;
  if (w && w.gsap) {
    gsapCore = w.gsap;
    ScrollTriggerPlugin = w.ScrollTrigger || ScrollTrigger2;
  } else {
    gsapCore = gsapWithCSS;
    ScrollTriggerPlugin = ScrollTrigger2;
  }
  if (!gsapCore.plugins?.ScrollTrigger) {
    gsapCore.registerPlugin(
      ScrollTriggerPlugin
    );
  }
  var gsap4 = gsapCore;
  var ScrollTrigger3 = ScrollTriggerPlugin;

  // src/utils/animation.ts
  function initStepAnimation() {
    const container = document.querySelector(".animation-container");
    const steps = Array.from(document.querySelectorAll(".animation-container .step"));
    if (!container || steps.length === 0) return;
    const heading = container.querySelector(".container-large");
    const setStepsTopOffset = () => {
    ...
    mm.add(
      ...
      (ctx) => {
        ...
        steps.forEach((s) => gsap4.set(s, { autoAlpha: 0, pointerEvents: "none" }));
        let current = 0;
        ...
        return () => {
          ScrollTrigger3.getAll().forEach((t) => t.kill());
        };
      }
    );
  }
  function showStep(step, immediate, isMobile) {
    const tl = gsap4.timeline({ defaults: { ease: "power2.out" } });
  ...
  function hideStep(step, isMobile) {
  ...
  // src/utils/gsap.ts
  function initGsapCardsAnimation() {
    gsap4.registerPlugin(ScrollTrigger3);
    ScrollTrigger3.matchMedia({
      "(min-width: 769px)": () => {
        const wrappers = document.querySelectorAll(".cards_wrapper");
        wrappers.forEach((wrapper) => {
          const cards = Array.from(wrapper.querySelectorAll(".cards_card"));
          if (cards.length === 0) return;
          gsap4.set(cards, { opacity: 0, y: 24 });
          const play = () => {
            gsap4.fromTo(
              cards,
              { opacity: 0, y: 24 },
  ...
  function initIntroDecorativAnimation() {
    ScrollTrigger3.matchMedia({
      "(min-width: 992px)": () => {
        const el = document.querySelector(".decorativ_btm-grid");
        const trigger = document.querySelector(".section_intro");
  ...
  window.Webflow.push(() => {
    ...
    if (hasAnim) {
      initStepAnimation();
      setTimeout(() => ScrollTrigger3.refresh(), 0);
      window.addEventListener("load", () => ScrollTrigger3.refresh(), { once: true });
    }
